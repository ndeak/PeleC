...
#pragma omp parallel
  ...
  if (level > 0) {
     ...
     MultiFab::Copy(*nE_crse, Coarse_State,UFS+E_ID,0,1,0);
     nE_crse->mult((1.0/me_g), 0, 1,0);
  ...
  // For nE system solve: A, alpha = 1     B = dt, beta = D_e
  ...
  {   
      const Box& bx = mfi.tilebox();
      const auto& nE_new = nE_state.array(mfi);
      const auto& nE_old = Sborder.array(mfi);
      const auto& nE_frc = forcing.array(mfi);
      amrex::ParallelFor(bx,
      [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
      {
        nE_frc(i,j,k) = me_g * (nE_new(i,j,k) - nE_old(i,j,k)) / dt;
      }); 
  }
